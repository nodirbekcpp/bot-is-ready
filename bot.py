...
TOKEN = "7834367258:AAEivjdI1roxgvnhEkgeKjln6_JP4TBY2HE" # BOT TOKEN
ADMIN_ID = 5929134791  # Admin ID 
    ...
    if os.path.exists(file_name):
        with open(file_name, "r", encoding="utf-8") as f:
            try:
                ...
                return {}
    # Fayl mavjud bo'lmasa, bo'sh dictionary qaytaradi
    return {}
...
# Load the list of admins from the JSON file
def load_admins():
    # Load the list of admins
    return load_json(ADMINS)

# Save the list of admins to the JSON file
...
class AddAdminState(StatesGroup):
    waiting_for_admin_id = State()  # State for waiting for the new admin ID

# Command to add a new admin
@dp.message(F.text == "/add_admin")
async def add_admin_command(message: types.Message, state: FSMContext):
    admins = load_admins()  # Load the list of admins from the JSON file
...
# Handle the new admin's ID and save it
@dp.message(AddAdminState.waiting_for_admin_id)
async def add_new_admin(message: types.Message, state: FSMContext):
    new_admin_id = message.text.strip()

    # Validate if the ID is numeric
    if not new_admin_id.isdigit():
        await message.answer("❌ ID noto'g'ri. Faqat raqamlarni kiriting.")
        return
    ...
    # Add the new admin to the list and save
    admins.append(new_admin_id)
    save_admins(admins)

    await message.answer(f"✅ Yangi admin ID {new_admin_id} muvaffaqiyatli qo'shildi!")
    await state.clear()
